...
public class ExtratorDadosNota8Test {
	
	String textoNota = "";
	{
		try {
			textoNota = new String(Files.readAllBytes(Paths.get(ExtratorDadosNota8Test.class.getResource("/teste8.txt").getPath())));
		} catch (IOException e) {
			e.printStackTrace();
		}	
	...
	public void deve_extrair_coo_corretamente() {
		...
		Assert.assertTrue(StringUtils.equals(dadosNota.getCoo(), "324916"));
	}
	...
	public void deve_extrair_valor_total_corretamente() {
		...
		Assert.assertTrue(StringUtils.equals(dadosNota.getValorTotal(), "20,40"));
	}
