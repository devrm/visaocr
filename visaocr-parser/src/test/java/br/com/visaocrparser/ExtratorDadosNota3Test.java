...
public class ExtratorDadosNota3Test {
	
	String textoNota = "";
	{
		try {
			textoNota = new String(Files.readAllBytes(Paths.get(ExtratorDadosNota3Test.class.getResource("/teste3.txt").getPath())));
		} catch (IOException e) {
			e.printStackTrace();
		}	
	...
	public void deve_extrair_coo_corretamente() {
		...
		Assert.assertTrue(StringUtils.equals(dadosNota.getCoo(), "324948"));
	}
	...
	public void deve_extrair_valor_total_corretamente() {
		...
		Assert.assertTrue(StringUtils.equals(dadosNota.getValorTotal(), "8,75"));
	}
